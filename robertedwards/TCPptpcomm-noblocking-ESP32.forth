...
\ Set up as TCP Client, with timeout  error (ms) if a conenction is not made with the remote Server within that period
: connect-nb                    ( timeoutperiod sock addr addrlen -- 0/err )
    MS-TICKS >R                 \ Read the start time
    BEGIN
        4DUP                    ( timeoutperiod sock1 addr addrlen timeoutperiod sock1 addr addrlen )
        connect                 ( timeoutperiod sock1 addr addrlen timeoutperiod sock2/err )
        swap r@ socktimeout?    ( timeoutperiod sock1 addr addrlen sock2/err )     \ throw an error if timed out
...
:noname
    ...
    begin
        ...
        0 recv-nb DUP 0< IF THROW THEN                          ( timeout [remsock] addr nreqd nrecvd )
...
:noname
    ...
    begin
        4dup                                                    ( timeout [remsock] addr nreqd timeout [remsock] addr nreqd )
        0 send-nb DUP 0< IF THROW THEN                          ( timeout [remsock] addr nreqd nrecvd )
        dup >r -                                                ( timeout [remsock] addr nreqd-nrecvd : nrecvd )
        swap r> + swap dup 0=                                   ( timeout [remsock] addr+nrcvd nreqd-nrecvd flag )
    until
    2drop 2drop                                                 ( -- )
...
: TEST1    ( -- )
    ...
    ['] (TEST1) catch 0<>
    IF
        socketError                                             \ if an error is thrown, decode and display
    ELSE
    WiFiDisconnect
;
